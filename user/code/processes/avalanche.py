...
def initialize(cfg, state):
    ...
    state.avalanche_balance = tf.Variable(tf.zeros_like(state.thk), dtype=tf.float32)
...
#                break
        ...
        # save the thickness before redistribution
        state.thk_preavalanche = state.thk
        state.thk = Ho + tf.where(H < 0, H, 0)
        # save the annual mass balance of the avalanche process as a state variable (in m/yr)
        state.avalanche_balance = (state.thk - state.thk_preavalanche) / cfg.processes.avalanche.update_freq
